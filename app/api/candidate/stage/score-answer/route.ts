...
import { scoreCurrentAnswer } from "@/lib/assessmentFlow";
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await scoreCurrentAnswer(candidate, provider));
  } catch (error) {
    return jsonError(error, "candidate_score_answer_failed");
  }
}
