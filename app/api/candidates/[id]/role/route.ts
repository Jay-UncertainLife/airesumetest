...
export async function POST(_: Request, { params }: { params: { id: string } }) {
  ...
  const roleAgents = store.agents.filter((agent) => agent.target_role === jobRole.name && agent.status === "enabled");
  const generatedPlan = await generateAbilityPlanWithOpenAI({
    targetRole: jobRole.name,
    agents: roleAgents.length ? roleAgents : store.agents.filter((agent) => agent.status === "enabled"),
